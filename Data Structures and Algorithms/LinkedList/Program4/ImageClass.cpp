...
// Return the RGB of a pixel of this Image class.
// Preconditions: Must have valid row and column inputed.
// Postconditions: return the RGB of a pixel from the coreImage at location (row ,column).
//				   return true if pixel is successfully found.
bool ImageClass::getPixel(const int& row, const int& col, int& red, int& green, int& blue)
{
	if (row >= getRows() || row < 0 || col >= getColumns() || col < 0)
	{
		return false;
	}
	red = coreImage.pixels[row][col].red;
	green = coreImage.pixels[row][col].green;
	blue = coreImage.pixels[row][col].blue;
	return true;
}
...
void ImageClass::setPixel(const int& row, const int& col, const int& red, const int& green, const int& blue)
{
	if (row < getRows() && row >= 0 && col < getColumns() && col >= 0)
	{
		coreImage.pixels[row][col].red = red;
		coreImage.pixels[row][col].green = green;
		coreImage.pixels[row][col].blue = blue;
	}
}
	...
	else if (coreImage.rows == in.getRows() && coreImage.cols == in.getColumns())
	{
		...
		for (int row = 0; row < coreImage.rows; row++)
		{
			for (int col = 0; col < coreImage.cols; col++)
			{
				...
				pixel inputImagePixel = in.coreImage.pixels[row][col];
